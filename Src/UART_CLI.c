...
static void prvUARTCommandConsoleTask(void *pvParameters)
{
	...
	for (;;)
	{
		...
		case '\r':
			...
			if (UART_CheckIdleState(&huart1) == HAL_OK && console_start == 1)
			{
				HAL_UART_Transmit(&huart1, (uint8_t *) pcNewLine, strlen(pcNewLine), strlen(pcNewLine));
			}
			...
			do
			{
				...
				if (console_start == 1 || command_always_print == 1)
				{
					HAL_UART_Transmit(&huart1, (uint8_t *) pcOutputString,
							strlen(pcOutputString), strlen(pcOutputString));
			...
			if (UART_CheckIdleState(&huart1) == HAL_OK && console_start == 1)
			{
				HAL_UART_Transmit(&huart1, (uint8_t *) pcEndOfCommandOutputString, strlen(pcEndOfCommandOutputString),
								  strlen(pcEndOfCommandOutputString));
			}
			break;
		case '\b':
